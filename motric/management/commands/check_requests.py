...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
    	...
    	warn_set = rds.filter(request_date__lt=time_width_w).filter(resolved=0)
    	danger_set = rds.filter(request_date__lt=time_width_d).filter(resolved=0)
    	table_w = '<table cellpadding="3" border="1" style="border:2px solid gray; border-collapse:collapse"><caption style="color:orange; font-size:14px; font-weight:bold; text-align:left; margin-bottom:5px">Requests that not responsed for ' + str(days_warn) + ' days (amount: ' + str(warn_set.count()) + ')</caption>'
    	for e in warn_set:
    		table_w += '<tr><td>' + str(e) + '</td><td>' + e.lab_location + '</td><td>https://' + motric_host + '/details/?t=r&pk=' + str(e.id) + '</td></tr>'
    	table_w += '</table>'

    	table_d = '<table cellpadding="3" border="1" style="border:2px solid gray; border-collapse:collapse"><caption style="color:red; font-size:14px; font-weight:bold; text-align:left; margin-bottom:5px">Requests that not responsed for ' + str(days_danger) + ' days (amount: ' + str(danger_set.count()) + ')</caption>'
    	for e in danger_set:
